...
def showValidation(type):
    ...
    if (min_fare is None and type == 'post') or min_fare == '':
        raise BuisnessValidationError(400, "SHOW002",
                                      "minimum Fare is required")
    ...
    if min_fare is not None:
        min_fare = verifyInterger(min_fare, 400, 'SHOW006',
                                  'minimum Fare should be integer')

        if min_fare <= 0:
            raise BuisnessValidationError(
                400, "SHOW007", "minimum Fare should be greater than 0")
    if duration is not None:
        duration = verifyInterger(duration, 400, 'SHOW008',
                                  'duration should be integer')

        if duration <= 0:
            raise BuisnessValidationError(400, "SHOW009",
    ...
    if is3d is not None:
        if not (is3d == 'True' or is3d == 'False'):
            ...
            raise BuisnessValidationError(
                400, "SHOW010",
                "is3d eithier true or false in lowercase as boolean")
...
class ShowAPI(Resource):
    ...
    def post(self):
        ...
        newShow = Show(show_name=show_name,
                       min_fare=min_fare,
                       duration=duration,
    ...
    def put(self, showId):
        ...
        if show_name is not None: currentShow.show_name = show_name
        if min_fare is not None: currentShow.min_fare = min_fare
        if duration is not None: currentShow.duration = duration
        if is3d is not None:
